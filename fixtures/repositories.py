...
def repository(user_data):
    ...
    res = gh_service.create_repository(random_str(), random_str(20), True, auto_init=True)
...
@pytest.fixture(scope="session")
def repository_no_init(user_data):
    gh_service = GitHubRestApi()
    res = gh_service.create_repository(random_str(), random_str(20), True, auto_init=False)
    assert res.status_code == 201
    res = res.json()
    yield res
    res = gh_service.delete_repository(user_data["login"], res["name"])
